...
class FaustCHOPUI : public APIUI {
 public:
  void addParameter(const char* label, FAUSTFLOAT* zone, FAUSTFLOAT init,
    ...
    // The substitutions here must be consistent with the `legal_chan_name`
    // method in script_build_ui.py

    std::string safeLabel = cleanLabel(label);

    APIUI::addParameter(safeLabel.c_str(), zone, init, min, max, step, type);
  ...
  void setParamValue(const std::string& path, FAUSTFLOAT v) {
    ...
    string p =
        path.length() > 0 && path[0] == '/' ? path : string("/TD/") + path;
  ...
  int getNumBarGraphs() { return m_mapIntToAddress.size(); }
